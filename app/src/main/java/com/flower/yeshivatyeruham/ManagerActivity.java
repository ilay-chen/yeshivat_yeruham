...
//this is edited  copy of sks activity. the only change is that the favorite isnt by sharedpreference, but write to file
...
//                }
                ...
                saveBtn.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent upload = new Intent(ManagerActivity.this, BackgroundService.class);
                        StartProgress();
...
//                    Toast.makeText(ctx, "לא נמצא נגן מתאים", Toast.LENGTH_SHORT).show();
                ...
                sc=true;
                Toast.makeText(ctx,"1,"+newPath, Toast.LENGTH_LONG).show();
                newPath=(newPath.substring(0,newPath.length()-4)).replace("^","/");
                Toast.makeText(ctx,"2,"+newPath, Toast.LENGTH_LONG).show();
                setAdapter();
                oldPath=newPath;
...
//            progress.show();
        ...
        protected String doInBackground(Integer... params) {
            ...
            PendingIntent pIntent = PendingIntent.getBroadcast(ctx, params[0], intent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                NotificationChannel uploadChannel = new NotificationChannel("myChannel", "Uploads", NotificationManager.IMPORTANCE_LOW);
                uploadChannel.setVibrationPattern(new long[]{0L});
                uploadChannel.enableVibration(true);
                uploadChannel.enableLights(false);
                uploadChannel.setDescription("הורדות");
                mNotifyManager.createNotificationChannel(uploadChannel);
            }
            ...
            try {
                ...
                if(isWIFIAvailable()) return downloadSMBfile(params[0], mBuilder);
                else if (isNetworkAvailable()) return downloadFTPfile(params[0], mBuilder);
                else return "err";
...
//                             .setStyle(new NotificationCompat.BigTextStyle().bigText(newPath.substring(newPath.lastIndexOf("/")+1)))
                            ...
                            .setSmallIcon(R.drawable.ic_notification)
                            .setProgress(0, 0, false);
                    mNotifyManager.notify(Integer.parseInt(s), mBuilder.build());
